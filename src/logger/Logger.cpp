...
void setup_logger(const spdlog::level::level_enum level) {
    ...
    console_sink->set_pattern("[%Y-%m-%dT%H:%M:%S.%e%z] [%^%=8l%$] [thread %t] [%s:%#] %v");
#ifdef _WIN32
    std::string s = (logger::log_folder / "pulp-ui.log").string();
    spdlog::sink_ptr file_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(s, FILE_ROTATION_TIME, 3);
#else  // _WIN32
    spdlog::sink_ptr file_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(logger::log_folder / "pulp-ui.log", FILE_ROTATION_TIME, 3);
#endif
    file_sink->set_pattern("[%Y-%m-%dT%H:%M:%S.%e%z] [%=8l] [thread %t] [%s:%#] %v");
